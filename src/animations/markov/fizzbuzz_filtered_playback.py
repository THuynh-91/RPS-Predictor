...
DATA_FILE = "../results/results_markov_vs_fizzbuzz.csv"
SHOW_ROUNDS = 2001
ANIMATED_ROUNDS = 5  # First 5 filtered rounds get animated
FINAL_ROUND_INDEX = -1  # Last filtered round gets animated
UPDATE_INTERVAL = 5  # Update every N filtered rounds
ROW_GAP = 0.35
...
class RPSPlayback(Scene):
    def construct(self):
        ...
        # FILTER: Only keep rounds divisible by 3, 5, or 15
        df_filtered = df[(df['round'] % 3 == 0) | (df['round'] % 5 == 0)].reset_index(drop=True)
        
        total_filtered_rounds = len(df_filtered)
        print(f"Filtered to {total_filtered_rounds} pattern rounds out of {SHOW_ROUNDS} total")

        # Title with filter info
        title = Text("Markov vs FizzBuzz - Pattern Rounds Only", font_size=32)
        subtitle = Text("(rounds ÷ 3, 5, or 15)", font_size=18, color=GRAY)
        ...
        scoreboard.next_to(title_group, DOWN, buff=0.5).to_edge(LEFT, buff=0.5)
        self.add(scoreboard)
        ...
        # Axes for accuracy graph (x-axis is now filtered round count)
        axes = Axes(
            x_range=[0, total_filtered_rounds, total_filtered_rounds//10],
            y_range=[0, 100, 20],
            x_length=4,
            y_length=3,
        ...
        x_label = Text("Pattern Rounds", font_size=16, color=GRAY)
        ...
        # Move history setup
        move_history_title, header_entry, move_history = self._setup_move_history(title_group)

        # Markov State Visualization
        markov_viz_placeholder = Text("Pattern: --", font_size=18, color=YELLOW)
        markov_viz_placeholder.next_to(title_group, DOWN, buff=0.5)
        ...
        # Main loop over FILTERED rounds
        for filtered_idx, row in df_filtered.iterrows():
            original_round = int(row["round"])
            filtered_round_num = filtered_idx + 1  # 1-indexed for display
            ...
            is_animated = (filtered_round_num <= ANIMATED_ROUNDS) or (filtered_round_num == total_filtered_rounds)
            should_update_visual = is_animated or (filtered_round_num % UPDATE_INTERVAL == 0)
            
            if filtered_round_num == ANIMATED_ROUNDS + 1:
                self.play(FadeIn(ff_text), run_time=0.3)
            
            if filtered_round_num == total_filtered_rounds and total_filtered_rounds > ANIMATED_ROUNDS:
                self.play(FadeOut(ff_text), run_time=0.3)

            if is_animated:
                # Show both the filtered index and original round number
                round_txt = Text(f"Pattern Round {filtered_round_num} (R{original_round})", font_size=26)
                round_txt.next_to(win_rate_center, DOWN, buff=0.5)
                self.play(FadeIn(round_txt), run_time=0.3)
                self._show_selection(round_txt, ai_move, opp_move, original_round)
            ...
            if should_update_visual:
                ...
                if len(player_history) >= 3:
                    ...
                    new_markov_viz.next_to(title_group, DOWN, buff=0.5)
                ...
                new_scoreboard.next_to(title_group, DOWN, buff=0.5).to_edge(LEFT, buff=0.5)
            ...
            if prediction and pd.notna(prediction):
                ...
                if should_update_visual:
                    ...
                    accuracy_data.append((filtered_round_num, accuracy_pct))
            ...
            if should_update_visual:
                history_entry = self._create_history_entry(original_round, ai_move, opp_move, result)
            ...
            if not is_animated and should_update_visual:
                self.wait(0.05)
            elif is_animated and filtered_round_num <= ANIMATED_ROUNDS:
                self.wait(0.1)
    ...
    def _create_markov_viz(self, history, transitions, prediction, ai_move):
        ...
        order_label = Text("Order k=3:", font_size=11, color=GRAY)
        order_label.next_to(history_display, LEFT, buff=0.3)
        pattern_row = VGroup(order_label, history_display)
        
        trans_bars = self._create_transition_bars(transitions)
        trans_bars.scale(0.9)
        trans_bars.next_to(pattern_row, DOWN, buff=0.25)
        
        # Calculate what Markov predicts based on current transitions
        if sum(transitions.values()) >= 2 and prediction != "--":
            # Find most common next move from transitions
            predicted_opp_move = max(transitions, key=transitions.get)
            # Show what we predict opponent will play → what AI counters with
        ...
        return VGroup(pattern_row, trans_bars, pred_text)
    ...
    def _setup_move_history(self, title_group):
        move_history_title = Text("Move History:", font_size=24, color=YELLOW)
        move_history_title.next_to(title_group, DOWN, buff=0.5).to_edge(RIGHT, buff=0.5)
    ...
    def _show_selection(self, round_txt, final_ai_move, final_opp_move, original_round):
        ai_label = Text("AI (Markov):", font_size=24, color=YELLOW).next_to(round_txt, DOWN, buff=0.5)
        
        # Show what FizzBuzz should play based on round number
        if original_round % 15 == 0:
            expected = "S (÷15)"
        elif original_round % 3 == 0:
            expected = "R (÷3)"
        elif original_round % 5 == 0:
            expected = "P (÷5)"
        else:
            expected = "?"
            
        opp_label = Text(f"Opponent (FizzBuzz): {expected}", font_size=20, color=YELLOW).next_to(ai_label, DOWN, buff=0.3)
        ...
        self.play(ai_options.animate.set_opacity(0.3), opp_options.animate.set_opacity(0.3), run_time=0.05)
        self.play(ai_options.animate.set_opacity(1), opp_options.animate.set_opacity(1), run_time=0.05)
