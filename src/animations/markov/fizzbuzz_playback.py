...
class RPSPlayback(Scene):
    def construct(self):
        ...
        # Title
        title = Text("Markov vs FizzBuzz (2k rounds)", font_size=36)
    ...
    def _create_markov_viz(self, history, transitions, prediction, ai_move):
        ...
        # History display (create first)
        history_display = self._create_history_display(history)
        
        # Order label (k=3) - positioned to the left of history
    ...
    def _show_selection(self, round_txt, final_ai_move, final_opp_move):
        ...
        opp_label = Text("Opponent (FizzBuzz):", font_size=24, color=YELLOW).next_to(ai_label, DOWN, buff=0.3)
