...
FINAL_ROUND = 2001        # animate the final round
...
class RPSQLearningPlayback(Scene):
    ...
    def construct(self):
        ...
        # Accuracy graph title + axes (placed left underneath scoreboard)
        ...
        axes = Axes(
            x_range=[0, SHOW_ROUNDS, max(1, SHOW_ROUNDS // 10)],
            y_range=[0, 100, 20],
            x_length=4,
            y_length=3,
        ...
        y_label = Text("Accuracy %", font_size=16, color=GRAY).next_to(axes, LEFT, buff=0.2).rotate(90 * DEGREES)
        ...
        for idx, row in df.iterrows():
            ...
            prediction = row.get("model_prediction", None) if "model_prediction" in row.index else None
            result = row["result"]  # "win" / "lose" / "tie"
            ...
            # Animated rounds: show selection animation
            if is_animated:
            ...
            # Update trackers (ValueTracker.set_value)
            if result == "win":
                win_tracker.set_value(win_tracker.get_value() + 1)
            elif result == "lose":
                loss_tracker.set_value(loss_tracker.get_value() + 1)
            else:
                tie_tracker.set_value(tie_tracker.get_value() + 1)

            # Visual scoreboard / win-rate block updates
            if should_update_visual:
                ...
                if is_animated:
                    self.play(Transform(scoreboard, new_scoreboard),
            ...
            # Prediction accuracy tracking (if present)
            if prediction is not None and pd.notna(prediction):
                total_predictions += 1  # uses outer-scoped var; will create here if first used
                # NOTE: ensure variables exist
                try:
                    correct_predictions
                except NameError:
                    correct_predictions = 0
                ...
                if should_update_visual:
                    ...
                    # Build a line representing accuracy
                    points = [axes.coords_to_point(x, y) for x, y in accuracy_data]
                    new_line = VMobject()
                    if len(points) >= 2:
                        new_line.set_points_as_corners(points)
                    else:
                        # a single point (make a tiny line)
                        p = points[0]
                        new_line.set_points_as_corners([p, p + RIGHT * 0.01])
            ...
            if should_update_visual:
                ...
                if len(move_history) < MAX_HISTORY:
                    idx_pos = len(move_history)
                    history_entry.next_to(header_entry, DOWN, buff=0.15 + idx_pos * ROW_GAP)
    ...
    def _setup_move_history(self, title):
        ...
        hdr_round_box = Rectangle(width=0.4, height=hdr_round_text.height * 1.3, stroke_width=0, fill_opacity=0)
        ...
        header_center_box = Rectangle(width=1.0, height=header_center_inner.height * 1.3, stroke_width=0, fill_opacity=0)
        ...
        result_header_box = Rectangle(width=0.75, height=result_header_text.height * 1.3, stroke_width=0, fill_opacity=0)
    ...
    def _mini_token_anim(self, move):
        circle = Circle(radius=0.25, color=MOVE_COLOR.get(move, WHITE), fill_opacity=0.3).set_stroke(width=2)
    ...
    def _create_history_entry(self, round_num, ai_move, opp_move, result):
        ...
        round_box = Rectangle(width=0.4, height=round_text.height * 1.3, stroke_width=0, fill_opacity=0)
        round_text.move_to(round_box)
        round_text.align_to(round_box, LEFT)
        round_col = VGroup(round_box, round_text)

        ai_circle = Circle(radius=0.12, color=MOVE_COLOR.get(ai_move, WHITE), fill_opacity=0.4).set_stroke(width=1.5)
        ...
        opp_circle = Circle(radius=0.12, color=MOVE_COLOR.get(opp_move, WHITE), fill_opacity=0.4).set_stroke(width=1.5)
        ...
        mid_box = Rectangle(width=1.0, height=mid_inner.height * 1.3, stroke_width=0, fill_opacity=0)
        mid_inner.move_to(mid_box)
        mid_col = VGroup(mid_box, mid_inner)

        res_text = Text(f"({result})", font_size=16, color=RESULT_COLOR.get(result, WHITE))
        res_box = Rectangle(width=0.75, height=res_text.height * 1.3, stroke_width=0, fill_opacity=0)
